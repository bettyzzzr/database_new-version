...
from services.itinerary_service import confirm_itinerary_booking, search_itinerary_options
...
def dashboard():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            record_search(session["user_id"], search["origin"], search["destination"], search["departure_date"])
            if not flights:
...
def itinerary():
    ...
    if request.method == "POST":
        try:
            legs = search_itinerary_options(booking_type, _raw_itinerary_legs())
            if any(not leg["flights"] for leg in legs):
                flash("One or more itinerary legs have no available flights.", "error")
        except ValueError as exc:
            flash(str(exc), "error")
    return render_template("customer_itinerary.html", legs=legs, booking_type=booking_type)
