...
def add_airport(staff, airport_code, airport_name, city_name):
    """Allow admin staff to add an airport under a city."""
    _require_admin(staff)
    _require_values(airport_code, airport_name, city_name)
    ...
    try:
        ...
        raise ValueError("Airport code already exists or city is invalid.") from exc
...
def create_flight(staff, flight_num, departure_airport, departure_time, departure_time_utc,
                  arrival_airport, arrival_time, arrival_time_utc, price, airplane_id):
    """Allow admin staff to create a scheduled flight for their own airline."""
    _require_admin(staff)
    _require_values(
        flight_num, departure_airport, departure_time, departure_time_utc,
        arrival_airport, arrival_time, arrival_time_utc, price, airplane_id,
    )
    airplane = fetch_one(
    ...
    try:
        execute(
            ...
            (
                ...
                _mysql_datetime(departure_time_utc),
                arrival_airport,
                _mysql_datetime(arrival_time),
                _mysql_datetime(arrival_time_utc),
                price, airplane_id,
            ),
        )
    except IntegrityError as exc:
